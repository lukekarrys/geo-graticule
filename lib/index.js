...
var addArray = function addArray(arr, arr2) {
...
var Geo = (function () {
  ...
  _createClass(Geo, [{
    ...
    key: 'getNeighboringGraticules',
    value: function getNeighboringGraticules() {
      var graticule = this.getGraticule();
      var ag = _partial2['default'](addArray, graticule);
      var neighbors = QUADRANTS[this.getQuadrant()].neighbors;

      return [[ag(neighbors[0][0]), ag(neighbors[0][1]), ag(neighbors[0][2])], [ag(neighbors[1][0]), ag(neighbors[1][1]), ag(neighbors[1][2])], [ag(neighbors[2][0]), ag(neighbors[2][1]), ag(neighbors[2][2])]];
    }
  }, {
    key: 'getNeighboringGraticuleBoxes',
    value: function getNeighboringGraticuleBoxes() {
      return this.getNeighboringGraticules().map(function (row) {
        return row.map(function (graticule) {
          return new Geo(graticule).getGraticuleBox();
        });
      });
    }
  }, {
